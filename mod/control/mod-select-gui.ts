...
function createModTextField(): TextFieldGuiElement {
  if (storage.modSelectGui!.modTextField?.valid) {
    return storage.modSelectGui!.modTextField
  }
  const modSelect = storage.modSelectGui!.modSelect
...
function createConfigGui(player: LuaPlayer): FrameGuiElement | undefined {
  ...
  storage.modSelectGui = { player } as typeof storage.modSelectGui
...
function destroyConfigGui() {
  if (!modSelectGuiValid()) return
  const configGui = storage.modSelectGui!
  storage.modSelectGui = undefined
...
function refreshConfigGui() {
  if (!modSelectGuiValid()) return
  const previousPlayer = storage.modSelectGui!.player
  destroyConfigGui()
